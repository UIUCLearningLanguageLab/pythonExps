...
def show_instructions(filePathName):
...
def prepare_pairs(item_data, config_dict):
    ...
    return item_data, trial_block_list
...
def experiment(assigned_item_data, trial_block_list, trial_event_list, config_dict):
    num_blocks = int(config_dict['BLOCKS'])
    
#    show_practice_instructions('main_instructions.txt')
#    for i in range(1, num_blocks+1):
#        show_instructions('block_instructions.txt')
#        block(trial_block_list[i-1], trial_event_list, i, config_dict)
#        if i < NUM_BLOCKS:
#            show_instructions("block_break.txt")
    ...
    for i in range(1, num_blocks+1):
        show_instructions('Stimuli/Instructions/block_instructions.txt')
        block(trial_block_list[i-1], trial_event_list, i,config_dict)
        if i < num_blocks:
...
def main():
    ...
    if verify_items_and_events(item_data, trial_event_list):
        assigned_item_data, trial_block_list = prepare_pairs(item_data, config_dict)
        experiment(assigned_item_data, trial_block_list, trial_event_list, config_dict)
